...
| [aws_ssm_parameter.secret_env_vars](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ssm_parameter) | data source |
...
| <a name="input_cloudwatch_log_retention_in_days"></a> [cloudwatch\_log\_retention\_in\_days](#input\_cloudwatch\_log\_retention\_in\_days) | Number of days to retain log events. | `number` | `null` | no |
| <a name="input_command"></a> [command](#input\_command) | The command that's passed to the container | `string` | `""` | no |
...
| <a name="input_environment_variables"></a> [environment\_variables](#input\_environment\_variables) | The environment variables to pass to the container | `map(string)` | `{}` | no |
...
| <a name="input_restart_policy"></a> [restart\_policy](#input\_restart\_policy) | The container restart policy and associated configuration parameters. When you set up a restart policy for a container, Amazon ECS can restart the container without needing to replace the task | `map(any)` | `{}` | no |
| <a name="input_secret_environment_variables"></a> [secret\_environment\_variables](#input\_secret\_environment\_variables) | SSM Parameter Store secrets to pass to the container, as name => parameter path under parameter\_path\_prefix. For more information, see [Specifying Sensitive Data](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data.html) in the Amazon Elastic Container Service Developer Guide | `map(string)` | `{}` | no |
| <a name="input_secretsmanager_environment_variables"></a> [secretsmanager\_environment\_variables](#input\_secretsmanager\_environment\_variables) | Secrets Manager secrets to pass to the container, as name => { secret\_arn, json\_key }. Omit json\_key to inject the whole secret value. Selecting a key needs Fargate platform version 1.4.0 or later | <pre>map(object({<br>  secret_arn = string<br>  json_key   = optional(string)<br>}))</pre> | `{}` | no |
...
| <a name="output_secretsmanager_secret_arns"></a> [secretsmanager\_secret\_arns](#output\_secretsmanager\_secret\_arns) | Bare Secrets Manager ARNs referenced by this container, for the execution role policy |
