...
data "aws_ssm_parameter" "ssm_env_vars" {
  for_each = var.ssm_environment_variables
...
resource "null_resource" "parameter_version_check" {
  triggers = {
    ssm_versions = jsonencode(
      {
        for parameter in data.aws_ssm_parameter.ssm_env_vars : parameter.name => parameter.version
      }
    )
  }
}
...
locals {
  ...
  container_secrets = concat(
    [for name, value in var.ssm_environment_variables : {
      name      = name
      valueFrom = data.aws_ssm_parameter.ssm_env_vars[name].name
    }],
